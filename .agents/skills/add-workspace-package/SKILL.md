...
description: Add or restructure a Shippy pnpm workspace app or package. Use when introducing a workspace, moving shared code across package boundaries, or defining package exports and dependencies.
---

# Add a Shippy Workspace
...
4. Create a private ESM manifest named under `@shippy/*`. Use `catalog:` for external versions and
...
6. Extend `@shippy/typescript-config` and add only the scripts the workspace can meaningfully run.
